module Categories.EmbGraph (
    ...
    createEmbGraph :: Cat o a -> (o -> String, a -> String) -> EmbGraph o a
    createEmbGraph cat toStringFunctor = 
        EmbGraph 
            (Cat (embGraphSource cat) (embGraphTarget cat) (embGraphId cat) (embGraphComp cat))
            (catToGraph cat toStringFunctor)
            where
